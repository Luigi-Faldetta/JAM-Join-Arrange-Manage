...
const newUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(201).json(resBody(true, null, safeUser, 'User created'));
  } catch (err: any) {
...
const getUser = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      return res.status(404).json(resBody(false, '404', null, 'No user found'));
    }

    const { password, ...safeUser } = { ...user.dataValues };
    res.status(200).json(resBody(true, null, safeUser, 'User fetched'));
  } catch (err: any) {
    process.env.NODE_ENV !== 'test' && console.error(err);
    res.status(400).json(resBody(false, '400', null, err.message));
  }
};
...
export const updateUser = async (req: Request, res: Response) => {
  try {
    ...
    if (!userExists) {
      return res.status(400).json(resBody(false, '400', null, 'Wrong user id'));
    }
    if (req.body.email) {
    ...
    const { password, ...safeUpdatedUser } = {
      ...updatedUser[1][0].dataValues,
    };
    res.status(200).json(resBody(true, null, safeUpdatedUser, 'User updated'));
  } catch (err: any) {
    process.env.NODE_ENV !== 'test' && console.error(err);
    res.status(500).json(resBody(false, '500', null, err.message));
  }
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json(resBody(true, null, deletedUser, 'User deleted'));
  } catch (err: any) {
    process.env.NODE_ENV !== 'test' && console.error(err);
    res.status(400).json(resBody(false, '400', null, err.message));
  }
...
const getAllUsers = async (req: Request, res: Response) => {
  try {
    ...
    if (userIds.length) {
      ...
      res.status(200).json(resBody(true, null, users, 'Event users fetched'));
    } else {
